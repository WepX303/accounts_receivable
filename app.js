...
(function () {
    ...
    var html = document.getElementsByTagName("HTML")[0];
    var lightDarkBtn = document.querySelectorAll(".light-dark-mode");
    var resizeEvent = new Event('resize'); // Eğer 'resizeEvent' tanımlı değilse bu satırı ekleyin.

    // Sayfa yüklendiğinde localStorage'dan tema tercihini kontrol et ve uygula
    const savedTheme = localStorage.getItem('theme');
    if (savedTheme) {
        html.setAttribute('data-bs-theme', savedTheme);

        // İkonu da başlangıçta doğru temaya göre güncelle
        if (lightDarkBtn && lightDarkBtn.length > 0) {
            const icon = lightDarkBtn[0].querySelector('i');
            if (icon) {
                if (savedTheme === 'dark') {
    ...
    if (lightDarkBtn && lightDarkBtn.length) {
        lightDarkBtn[0].addEventListener("click", function (event) {
            ...
            if (html.hasAttribute("data-bs-theme") && html.getAttribute("data-bs-theme") == "dark") {
                // Temayı light yap
                // setLayoutMode("data-bs-theme", "light", "layout-mode-light", html); // Eğer bu fonksiyon gereksizse kaldırın
                html.setAttribute("data-bs-theme", "light");
                newTheme = "light";
                this.querySelector('i').classList.remove('bx-sun');
                this.querySelector('i').classList.add('bx-moon');
            } else {
                // Temayı dark yap
                // setLayoutMode("data-bs-theme", "dark", "layout-mode-dark", html); // Eğer bu fonksiyon gereksizse kaldırın
                html.setAttribute("data-bs-theme", "dark");
                newTheme = "dark";
                this.querySelector('i').classList.remove('bx-moon');
                this.querySelector('i').classList.add('bx-sun');
            }

            // Seçilen temayı localStorage'a kaydet
            localStorage.setItem('theme', newTheme);

            // Temanın kendi resize event'ini çağır
            window.dispatchEvent(resizeEvent);
        });
    }
	}
