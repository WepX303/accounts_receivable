                    ...
                    <table class="table align-middle table-nowrap mb-0" id="customerTable">
                        ...
                        @forelse($credit_users_info as $credit)
                            @php
                                ...
                                $isActive = isset($selected) && $selected && (int)$selected->logicalref === (int)$credit->logicalref;
                                ...
                                $amountLocalText  = $amountLocal !== null ? number_format((float)$amountLocal, 2) : '';
                                $amountCenterText = $amountCenter !== null ? number_format((float)$amountCenter, 2) : '';

                                $paidLocalText    = $paidLocal !== null ? number_format((float)$paidLocal, 2) : '';
                                $paidCenterText   = $paidCenter !== null ? number_format((float)$paidCenter, 2) : '';

                                $willPayText = optional($credit->willpaiddate)->format('Y-m-d') ?? '';
                                $lastNoteText = optional($credit->lastnoteddate)->format('Y-m-d H:i:s') ?? '';
                            @endphp

                            <tr
                                class="js-credit-row {{ $isActive ? 'table-active' : '' }}"
                                style="cursor:pointer;"
                                data-id="{{ $credit->logicalref }}"
                            ...
                            >
                                <th scope="row">
                                    <div class="form-check">
                                        <input class="form-check-input js-row-check" type="checkbox" name="chk_child">
                                    </div>
                                </th>
                                ...
                                {{-- Amount --}}
                                <td class="text-end">
                                    <div class="fw-medium {{ $credit->amount_local !== null ? 'text-primary' : '' }}">
                                ...
                                {{-- Paid --}}
                                <td class="text-end">
                                    <div class="fw-medium {{ $credit->paid_local !== null ? 'text-primary' : '' }}">
                                ...
                                <td>
                                    <span class="badge {{ !empty($credit->active) ? 'bg-success-subtle text-success' : 'bg-danger-subtle text-danger' }}">
                    ...
                    <table class="table table-borderless mb-0">
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td id="d_willpaiddate">{{ $selected ? (optional($selected->willpaiddate)->format('Y-m-d') ?? '-') : '-' }}</td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td id="d_active">{{ $selected ? (!empty($selected->active) ? 'Active' : 'Blok') : '-' }}</td>
                            </tr>
                            <tr>
                                <td class="fw-medium">Last Note Date</td>
                                <td id="d_lastnoteddate">{{ $selected ? (optional($selected->lastnoteddate)->format('Y-m-d H:i:s') ?? '-') : '-' }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
