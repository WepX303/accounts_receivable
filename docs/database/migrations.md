...
- [credits](#6-credits-tablosu)
- [sync_state](#7-sync_state-tablosu)
- [avshocrecat_report (pgsql)](#8-avshocrecat_report-tablosu-pgsql)
- [credits ek alanlar](#9-credits-ek-alanlar-local-paid--amount)
- [credit_payments](#10-credit_payments-tablosu)
- [credits amount tipi değişimi](#11-credits-amount-tip-değişimi)
...
## 9) `credits` Ek Alanlar (Local Paid / Amount)

Bu bölüm, `credits` tablosuna sonradan eklenen “local override” alanlarını açıklar.

### 9.1 Paid Local Alanları

**Migration:** `2026_01_20_122800_add_paid_local_fields_to_credits_table.php`
...
**Migration:** `2026_01_20_123743_add_amount_local_fields_to_credits_table.php`
...
- `credit_logicalref` (unsignedBigInteger, index) → FK `credits.logicalref`
...
- `credit_logicalref` → `credits.logicalref` (cascadeOnDelete)
...
## 11) `credits` Amount Tip Değişimi

**Migration:** `2026_01_23_112947_change_amount_type_in_credits_table.php`

### Amaç

`credits.amount` alanını daha güvenli finans tipi olan `decimal(18,2)` yapmaya dönüştürür.
...
- Müşteri bilgileri `credits` tablosunda değişse bile geçmiş ödeme kayıtları etkilenmez.
